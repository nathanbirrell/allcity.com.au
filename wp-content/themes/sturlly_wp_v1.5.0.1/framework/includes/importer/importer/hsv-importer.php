 ...
 * @since 2.2.0
 *
 * @category HSV Framework
 * @package  new
 * @author   relstudiosnx
 * @link     http://relstudiosnx.com/
 *
 */
class HSV_Theme_Importer {
    ...
    public function add_admin() {
        ...
        add_submenu_page('themes.php', "Import Demo Data", "Import Demo Data", 'switch_themes', 'hsv_demo_installer', array($this, 'demo_installer'));
    ...
    public function demo_installer() {
        ...
        <form method="post">
          <p><label><strong>Choose Demo: </strong></label></p>
          <p>
            <select name="demo_xml">
          ...
          <input type="hidden" name="demononce" value="<?php echo wp_create_nonce('hsv-demo-code'); ?>" />
        ...
        <?php
		    $demo_xml = ( isset($_POST['demo_xml'])) ? $_POST['demo_xml']:'agency';
		    $action = isset($_REQUEST['action']) ? $_REQUEST['action'] : '';
		
        if( 'demo-data' == $action && check_admin_referer('hsv-demo-code' , 'demononce')){

            $this->set_demo_data( $this->content_demo.$demo_xml.'.xml' );
    ...
    public function set_demo_theme_options( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			wp_die(
				__( 'Theme options Import file could not be found. Please try again.', 'hsv' ),
				'',
		...
		if ( empty( $data ) || ! is_array( $data ) ) {
			wp_die(
				__( 'Theme options import data could not be read. Please try a different file.', 'hsv' ),
				'',
				array( 'back_link' => true )
			);
		}
		...
		// Hook before import
		$data = apply_filters( 'hsv_theme_import_theme_options', $data );
    ...
    function available_widgets() {
    	...
    	return apply_filters( 'hsv_theme_import_widget_available_widgets', $available_widgets );
    ...
    function process_widget_import_file( $file ) {
    	...
    	if ( ! file_exists( $file ) ) {
    		wp_die(
    			__( 'Widget Import file could not be found. Please try again.', 'hsv' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    ...
    public function import_widgets( $data ) {
    	...
    	if ( empty( $data ) || ! is_object( $data ) ) {
    		wp_die(
    			__( 'Widget import data could not be read. Please try a different file.', 'hsv' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    	...
    	// Hook before import
    	$data = apply_filters( 'hsv_theme_import_widget_data', $data );
    	...
    	foreach ( $data as $sidebar_id => $widgets ) {
    		...
    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
    			...
    			$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'hsv' );
    		}
    		...
    		foreach ( $widgets as $widget_instance_id => $widget ) {
    			...
    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
    				...
    				$widget_message = __( 'Site does not support widget', 'hsv' ); // explain why widget not imported
    			}

    			// Filter to modify settings before import
    			// Do before identical check because changes may make it identical to end result (such as URL replacements)
    			$widget = apply_filters( 'hsv_theme_import_widget_settings', $widget );
    			...
    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
    				...
    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
    					...
    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
    						...
    						$widget_message = __( 'Widget already exists', 'hsv' ); // explain why widget not imported
    			...
    			if ( ! $fail ) {
    				...
    				if ( $sidebar_available ) {
    					...
    					$widget_message = __( 'Imported', 'hsv' );
    				} else {
    					$widget_message_type = 'warning';
    					$widget_message = __( 'Imported to Inactive', 'hsv' );
    				}
    			...
    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'hsv' ); // show "No Title" if widget instance is untitled
    	...
    	// Hook after import
    	do_action( 'hsv_theme_import_widget_after_import' );

    	// Return results
    	return apply_filters( 'hsv_theme_import_widget_results', $results );
